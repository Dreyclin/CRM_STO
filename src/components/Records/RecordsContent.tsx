...
const RecordsContent: React.FC = () => {
    ...
    const displayedDays = limitedDays && limitedDays.slice(startIndex, endIndex);

    return (
        <div className="content d-flex flex-column gap-4 mb-4">
            {displayedDays && displayedDays.map(day => {return <Day day={day}/>})}
            <RecordsFooter onNext={handleNext} onPrevious={handlePrevious} hasNext={days && endIndex < days?.length} hasPrevious={currentPage > 0}/>
        </div>
    )
}
