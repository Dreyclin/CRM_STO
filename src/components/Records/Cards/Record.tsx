...
const Record: React.FC<RecordProps> = ({duration, car, description, status, id, client, day, totalCost}) => {
    const {getBadgeClass, handleStatusClick, options, isOptionsOpen, toggleOptions} = useRecordStatus();
    const {handleServiceClick, services, toggleServices, isServicesOpen} = useRecordsServices();
        ...
        <div className="card w-25">
            <div className="card-header d-flex align-items-center justify-content-between">
                ...
                <div className="badges d-flex gap-2">
                    <div className={getBadgeClass(status)} onClick={() => {toggleOptions(); isServicesOpen && toggleServices()}}>{status}</div>
                    <div className="badge bg-primary status-badge" onClick={() => {toggleServices(); isOptionsOpen && toggleOptions()}}>Услуги</div>
                    <div className="d-flex flex-column record-status">
                    {(services && isServicesOpen) && services.map(option => <div className="status-item p-2" onClick={() => {handleServiceClick(id, day, {serviceName: option.service, cost: option.cost}); toggleServices()}}>{option.service} - {option.cost}</div>)}
            ...
            <div className="card-body">
                ...
                {totalCost !== 0 && <p className="fw-bold">Итого: {totalCost}</p>}
