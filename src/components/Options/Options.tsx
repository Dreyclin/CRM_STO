...
const Options: React.FC = () => {
    ...
    const handleAddStatus = (statusRecord: string) => {
        if (statusRecord !== "") {
            ...
            dispatch(addStatusRecord(creds))
                .unwrap()
                .catch((err) => {
                    alert(err);
                });
    ...
    return (
        <div className="options-container w-100 d-flex gap-5 flex-wrap">
            <div className="option-container d-flex flex-column">
                <h5>Додавання нового статуса заявки</h5>
                <div className="status-record-option d-flex flex-column gap-3">
                    <input
                        type="text"
                        className="form-control"
                        placeholder="Новий статус заявки"
                        value={statusRecord}
                        onChange={(e) => setStatusRecord(e.target.value)}
                    />
                    <button
                        className="btn btn-primary"
                        onClick={() => handleAddStatus(statusRecord)}
                    >
                        Додати
                    </button>
                </div>
                    ...
                    <div className="d-flex flex-column gap-3">
                        <div className="service-inputs d-flex gap-3 justify-content-between">
                            ...
                            <input
                                ...
                                value={serviceCost}
                                onChange={(e) =>
                                    setServiceCost(Number(e.target.value))
                                }
                            />
                        </div>
                        <button
                            className="btn btn-primary"
                            onClick={() =>
                                handleAddService(service, serviceCost)
                            }
                        >
                            Додати
                        </button>
